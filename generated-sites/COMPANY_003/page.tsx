...
export default function GeneratedHomepage() {
  return (
    <main data-template="product_basic" data-asset-theme={assets.asset_theme}>
      ...
      <section data-section="product_area"><h2>상품 안내</h2>{content.products.length > 0 ? content.products.map((product) => <article key={product.name}><h3>{product.name}</h3><p>{product.description}</p></article>) : <p>{content.product_registration_cta}</p>}</section>
